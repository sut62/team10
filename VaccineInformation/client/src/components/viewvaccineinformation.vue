        ...
        <v-data-table :headers="headers" :items="items" :items-per-page="5" class="elevation-1" :search="search">
            ...
            <v-col cols="3">
              <v-text-field
                ...
                v-model="vaccineinformation.listid"
              ></v-text-field>
                 <p v-if="vaccineCheck != ''">ID ข้อมูลที่จะทำการลบ : {{listId}}
                        <v-btn class="" @click="deleteVaccineInformation" color="#D50000" style="color:#FFFFFF" >ลบ</v-btn>
                 </p>
              </v-col>
            
            <v-col cols="2">
              <div class="">
                 <v-btn @click="findVaccineInformation" depressed large color="#000000" style="color:#FFFFFF;">ยืนยัน</v-btn>
    ...
    return {
      ...
      headers: [
        ...
        { text: "วันที่หมดอายุ", value: "expiredate" },
         { text: "ทำรายการโดย", value: "medicalStaff.fullname" },
  ...
  methods: {
    /* eslint-disable no-console */
     ...
     findVaccineInformation() {
      http
        .get("/vaccineinformation/" + this.vaccineinformation.vaccineinformationid)
        .then(response => {
    ...
    deleteVaccineInformation() {
      http
        .delete("/vaccineinformation/" + this.vaccineinformation.vaccineinformationid)
        .then(response => {
